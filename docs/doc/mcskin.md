---
api: mcskin
---

# MC 正版皮肤披风获取

> 可以获取到正版 MC 用户名的皮肤详情

## 接口地址

```http-vue
https://api.nxvav.cn/api/{{$frontmatter.api}}/
```

## 请求示例

::: code-group

<!--@include: ./example/code.md-->

:::

## 请求参数

| 参数名  | 类型   | 必填 | 说明                                                 |
| ------- | ------ | ---- | ---------------------------------------------------- |
| id/uuid | string | 是   | 正版 MC 用户名/正版 MC 玩家的 UUID，优先级 id > uuid |
| type    | string | 否   | 输出类型，可选值`skin_url`，`skin_cloak`             |

## 返回响应

| 字段名                | 类型   | 说明                               |
| --------------------- | ------ | ---------------------------------- |
| code                  | string | 状态码                             |
| name                  | string | 玩家昵称                           |
| uuid                  | string | 玩家 UUID                          |
| properties            | string | 玩家纹理属性                       |
| Yggdrasil             | string | 玩家 Yggdrasil 私钥签名            |
| skin\_                | string | 皮肤数据                           |
| skin\_.skin_timestamp | string | 调用纹理数据的时间                 |
| skin\_.skin_url       | string | 玩家自定义皮肤，没有为 null        |
| skin\_.skin_cloak     | string | 玩家披风，没有或没装备为 null      |
| skin\_.skin_model     | string | 玩家皮肤模型，null 为男，slim 为女 |

## 返回示例

::: code-group

```json [成功 200]
{
  "code": 200,
  "name": "nuoxian",
  "uuid": "91cb793afa4a4684973fe7955e679628",
  "properties": "textures",
  "Yggdrasil": "",
  "skin_": {
    "skin_timestamp": 1663251309055,
    "skin_url": "http://textures.minecraft.net/texture/558a3edc9adab15dfe7ac6124b0177abdb4fa856e926c2ec6d5d9b55210849ec",
    "skin_cloak": null,
    "skin_model": null
  }
}
```

```json [失败 400]
{
  "code": 400,
  "msg": "查询失败"
}
```

:::
